...
path <- file.path('C:/LOCAL/ICF/R03 SAE/test/data')   
#===============================================================================
#.- Obtain parameters for simulation from case data
#===============================================================================
 
load(file.path(path,'ldat.r'))
load(file.path(path,'gs'))
load(file.path(path,'W' ))
...
#.- total number of periods (there are actually 2, parallel to T0s) 
...
#.- population at risk in each county and year (1)
...
#.- (scaled) matrix with the neighborhood structure (1)
case.par$gs <- gs
...
#.- Number of treated units desired.   (2)
...
#!the series for suicide is longer than for hospitlaizations
#!T0 is therefore diffrent

#.- (residual) variance of log relative risk (we mention only 1)
...
#.- spatial dependence ('mixing') parameter (3)
case.par$phi <- c(IP=.1,S=.3,.5)
#.- serial correlation parameter  (3)  
