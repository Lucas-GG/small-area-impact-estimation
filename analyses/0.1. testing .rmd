...
is_test_set <- is.na(dt$y0) & !is.na(dt$y)
table(is_test_set)
...
# Forest
```{r}
dt |> impute_forest(ntrees = 20, nboot = 20, forest_cores = 16)
forest20 <- dt |>
  score(idx = is_test_set) |>
  summ_score()

dt |> impute_forest(ntrees = 40, nboot = 40, forest_cores = 16)
forest40 <- dt |>
  score(idx = is_test_set) |>
  summ_score()

dt |> impute_forest(ntrees = 200, nboot = 200, forest_cores = 16)
forest <- dt |>
  score(idx = is_test_set) |>
  summ_score()
...
rbind(pm, forest20,  forestx) |> round(3)
```
