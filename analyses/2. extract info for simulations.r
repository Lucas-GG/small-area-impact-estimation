...
length(unique(ldat$year[!is.na(ldat$S)]))
length(unique(ldat$year[!is.na(ldat$Y)]))
case.par$T <- 9


#3.- population at risk in each county and year (1)
n0 <- reshape(ldat[ldat$otyp=='IP' & ldat$year >=2008 & ldat$opop=='youth',c('id','year','P')],
                v.names = 'P', idvar = 'id',
...
n0_a <- reshape(ldat[ldat$otyp=='IP' & ldat$year >=2008 & ldat$opop=='adult',c('id','year','P')],
                v.names = 'P', idvar = 'id',
                timevar = 'year', direction = 'wide')
case.par$n_a <- n0_a[,-1]
rownames(case.par$n_a)<- n0_a$id
colnames(case.par$n_a)<- 2008:2016
head(case.par$n_a)
...
with(ldat ,
  tapply(Y,list(outcome,year),sum,na.rm=T)/tapply(P,list(outcome,year),sum,na.rm=T))*1000

with(ldat[ldat$year>=2008,],
        tapply(Y,list(outcome),sum,na.rm=T)/tapply(P,list(outcome),sum,na.rm=T))*1000

with(ldat[ldat$otyp=='IP',] ,
  tapply(S,list(outcome,year),sum,na.rm=T)/tapply(P,list(outcome,year),sum,na.rm=T))*100000

with(ldat ,
        tapply(S,list(outcome),sum,na.rm=T)/tapply(P,list(outcome),sum,na.rm=T))*100000

case.par$r0  <- c(IP=2.9/1000,S=8.9/100000)
case.par$r0

case.par$r0_a  <- c(IP=2.3/1000,S=18/100000)
case.par$r0_a
...
# 'residual': after taking systematic trends into account
# IP is more varible than suicide beore taking into account trends
...
#load('data/inla/Yspya.s.bart')
#summary(Yspya.s.bart)

 mean(c(1/7.617, 1/ 7.179))
case.par$sigma2_a <- .135
...
case.par$urb13 <- ldat[ldat$otyp=='IP' & ldat$opop=='youth' & ldat$year ==2008,c('id','urb13')]
table(case.par$urb13$urb13)

summary(case.par)

path <- 'C:/LOCAL/ST_evaluation'
save(case.par,file=file.path(path,'data/case.par'))
...
map <- merge(map,tigris::fips_codes[,c('state_code','county_code','state','state_name')],
  by.x=c('STATEFP','COUNTYFP'),
  by.y=c('state_code','county_code'),all.x=T)
table(map$state)
...
toy.par <- within(toy.par,{
  ...
  n_a   <- case.par$n_a[paste(smap$id),]
  N   <- dim(n)[1]
  T   <- dim(n)[2]
